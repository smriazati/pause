      ...
      FunctionDeclaration(p) {
        ...
        schema.tags = (_b = schema.tags) == null ? void 0 : _b.filter((tag) => {
          ...
          if (tag.startsWith("@returns")) {
            const { type } = ((_a2 = tag.match(/^@returns\s+\{(?<type>.+)\}/)) == null ? void 0 : _a2.groups) || {};
            if (type) {
          ...
          if (tag.startsWith("@param")) {
            const { type, param } = ((_b2 = tag.match(/^@param\s+\{(?<type>.+)\}\s+(?<param>\w+)/)) == null ? void 0 : _b2.groups) || {};
            if (type && param) {
  ...
  if (firstTag >= 0) {
    ...
    for (const tag of tags) {
      if (tag.startsWith("@type")) {
        const type = (_a = tag.match(/@type\s+\{(.+)\}/)) == null ? void 0 : _a[1];
        Object.assign(schema, getTypeDescriptor(type));
        const typedef = tags.find((t2) => {
          var _a2;
          return ((_a2 = t2.match(/@typedef\s+\{(.+)\} (.*)/)) == null ? void 0 : _a2[2]) === type;
        });
        if (typedef) {
          schema.markdownType = type;
          schema.tsType = (_b = typedef.match(/@typedef\s+\{(.+)\}/)) == null ? void 0 : _b[1];
        }
        continue;
      }

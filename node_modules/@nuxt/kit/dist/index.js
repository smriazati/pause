...
function requireModulePkg(id, opts = {}) {
  return requireModule(path.join(id, "package.json"), opts);
}
...
const _app = {
  ...
  app: {
    $resolve: (val, get) => {
      const useCDN = ufo.hasProtocol(get("build.publicPath"), true) && !get("dev");
      const isRelativePublicPath = ufo.isRelative(get("build.publicPath"));
      return defu__default(val, {
        basePath: get("router.base"),
        assetsPath: isRelativePublicPath ? get("build.publicPath") : useCDN ? "/" : ufo.joinURL(get("router.base"), get("build.publicPath")),
  ...
  appTemplatePath: {
    $resolve: (val, get) => {
      if (val) {
        return path.resolve(get("srcDir"), val);
      }
      if (fs.existsSync(path.join(get("srcDir"), "app.html"))) {
        return path.join(get("srcDir"), "app.html");
      }
      return path.resolve(get("buildDir"), "views/app.template.html");
    }
  },
  store: {
    $resolve: (val, get) => val !== false && fs.existsSync(path.join(get("srcDir"), get("dir.store"))) && fs.readdirSync(path.join(get("srcDir"), get("dir.store"))).find((filename) => filename !== "README.md" && filename[0] !== ".")
  },
  vueMeta: null,
  head: {
    meta: [],
...
const _common = {
  rootDir: {
    $resolve: (val) => typeof val === "string" ? path.resolve(val) : process.cwd()
  },
  srcDir: {
    $resolve: (val, get) => path.resolve(get("rootDir"), val || ".")
  },
  buildDir: {
    $resolve: (val, get) => path.resolve(get("rootDir"), val || ".nuxt")
  },
  ...
  globals: {
    ...
    readyCallback: (globalName) => `on${scule.pascalCase(globalName)}Ready`,
    loadedCallback: (globalName) => `_on${scule.pascalCase(globalName)}Loaded`
  },
  serverMiddleware: {
    $resolve: (val) => {
      if (!val) {
  ...
  modulesDir: {
    ...
    $resolve: (val, get) => [].concat(val.map((dir) => path.resolve(get("rootDir"), dir)), path.resolve(process.cwd(), "node_modules"))
  },
  dir: {
    assets: "assets",
    app: "app",
  ...
  alias: {
    $resolve: (val, get) => ({
      ...
      "~": get("srcDir"),
      "@": get("srcDir"),
      [get("dir.assets")]: path.join(get("srcDir"), get("dir.assets")),
      [get("dir.static")]: path.join(get("srcDir", get("dir.static"))),
      ...val
    })
  },
...
const build = {
  ...
  publicPath: {
    $resolve: (val, get) => {
      if (ufo.hasProtocol(val, true) && get("dev")) {
        val = null;
      }
...
const router = {
  mode: "history",
  base: {
    $resolve: (val = "/") => ufo.withTrailingSlash(ufo.normalizeURL(val))
  },
...
const generate = {
  dir: {
    $resolve: (val = "dist", get) => path.resolve(get("rootDir"), val)
  },
  routes: [],
  exclude: [],
  concurrency: 500,
  ...
  staticAssets: {
    dir: "static",
    base: { $resolve: (val, get) => val || ufo.joinURL(get("app.assetsPath"), get("generate.dir")) },
    versionBase: { $resolve: (val, get) => val || ufo.joinURL(get("generate.base"), get("generate.version")) },
...
function loadNuxtConfig(opts) {
  ...
  const nuxtConfigFile = tryResolveModule(path.resolve(rootDir, opts.configFile || "nuxt.config"));
  let nuxtConfig = {};
  ...
  return untyped.applyDefaults(nuxtConfigSchema, nuxtConfig);
}
...
const nuxtCtx = unctx.getContext("nuxt");
...
function resolvePath(path, opts = {}) {
  ...
  resolvedPath = upath.resolve(opts.base || ".", resolvedPath);
  ...
  for (const ext of opts.extensions) {
    ...
    if (!isDirectory && fs.existsSync(resolvedPathwithExt)) {
      return resolvedPathwithExt;
    }
    const resolvedPathwithIndex = upath.join(resolvedPath, "index" + ext);
...
async function resolveFiles(path, pattern) {
  const files = await globby__default(pattern, {
    cwd: path,
    followSymbolicLinks: true
  });
  return files.map((p) => upath.resolve(path, p));
}
...
exports.addErrorLayout = addErrorLayout;
exports.addLayout = addLayout;
exports.addPlugin = addPlugin;
exports.addServerMiddleware = addServerMiddleware;
...
exports.extendBuild = extendBuild;
